...
mutable struct Model <: ModiaMath.AbstractSimulationModel
    ...
    function Model(;L=1.0, m=1.0, d=0.1, g=9.81, phi0_deg=90, structureOfDAE = ModiaMath.DAE_NoSpecialStructure)
        @assert(L > 0.0)
        @assert(m > 0.0)
        @assert(d >= 0.0)
        simulationState = ModiaMath.SimulationState("PendulumODE", getModelResidues!, [phi0_deg * pi / 180; 0.0], getVariableName; 
                                                    structureOfDAE = structureOfDAE, nw=2)
...
function getModelResidues!(m::Model, t::Float64, _x::Vector{Float64}, _derx::Vector{Float64}, _r::Vector{Float64}, _w::Vector{Float64})  
    phi  = _x[1]
    w    = _x[2]  
    derw = (-m.m * m.g * m.L * sin(phi) - m.d * w) / (m.m * m.L^2)
    _r[1] = w    - _derx[1]
    _r[2] = derw - _derx[2]
