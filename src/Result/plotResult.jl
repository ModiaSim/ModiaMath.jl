...
- `minXaxisTickLabels::Bool`: = true, if xaxis tick labels are removed in a vector or array of plots,
  if not the last row (useful when including plots in a document).
...
plot(result, names::Symbol; heading::AbstractString="", grid::Bool=true, xAxis="time", figure::Int=1, prefix::AbstractString="", reuse::Bool=false, maxLegend::Integer=10,
    minXaxisTickLabels::Bool=false) =
    plot(result, string(names), heading=heading, grid=grid, xAxis=string(xAxis), figure=figure, prefix=prefix, reuse=reuse, maxLegend=maxLegend, minXaxisTickLabels=minXaxisTickLabels)
...
plot(result, names::AbstractVector; heading::AbstractString="", grid::Bool=true, xAxis="time", figure::Int=1, prefix::AbstractString="", reuse::Bool=false, maxLegend::Integer=10,
    minXaxisTickLabels::Bool=false) =
    plot(result, reshape(names, length(names), 1); heading=heading, grid=grid, xAxis=string(xAxis), figure=figure, prefix=prefix, reuse=reuse, maxLegend=maxLegend, minXaxisTickLabels=minXaxisTickLabels)
