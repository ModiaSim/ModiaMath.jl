...
{
    "location": "index.html#Version-0.5.1-1",
    "page": "Home",
    "title": "Version 0.5.1",
    "category": "section",
    "text": "New example ModiaMath/examples/Simulate_PendulumDAE (pendulum as index 3 system with variables \"x\" and \"y\" (and not angle). Transformation to index 1 form with the Gear-Gupta-Leimkuhler transformation.Minor errors in Print/log output of lambda/mue variables corrected."
},

{
    "location": "index.html#Version-0.5.0-1",
    "page": "Home",
    "title": "Version 0.5.0",
    "category": "section",
    "text": "Version is not backwards compatible:\nThe values of @enum structureOfDAE have changed.\nModiaMath.SimulationState: Keyword argument structureOfDAE has default DAE_LinearDerivativesAndConstraints instead of DAE_NoSpecialStructure. The consequence is that by default it is assumed that the DAE depends linearly on the derivatives (this should probably always be the case, so this should be uncritical). However, the constraint equations (fc) must be now precisely identified via the new keyword argument is_constraint.\nModiaMath.SimulationState: Keyword argument nc has no effect anymore (is ignored). Instead with new keyword argument is_constraint it is defined which residue element is a constraint (fc) equation. At an event instant, the value of is_constraint can change. This allows to model varying index systems (with Dirac impulses).ModiaMath.SimulationState: New keyword argument has_constraintDerivatives. If true, the derivatives of the constraint equations (der(fc)) must be returned in the model residue, if ModiaMath.compute_der_fc = true. During initialization/reinitialization the Jacobian that is used to compute der(x) is then no longer computed with finite differences but by using der(fc) which gives a more reliable numerical solution.New function ModiaMath.get_is_constraint to get a reference to the is_constraint vector.New function ModiaMath.compute_der_fc to inquire in a model whether the constraint equations (fc) or its derivatives (der(fc)) shall be computed, if has_constraintDerivative was defined to be true. This feature is demonstrated in model examples/withoutMacros_withoutVariables/models/PendulumDAE.jl.New example examples/withoutMacros_withoutVariables/Simulate_IdealClutch.jl to simulate an electrical circuit driving an inertia that is connected with an ideal clutch to a second inertia. It is demonstrated how the index can change during simulation and how Dirac impulses are handled during initialization and at events.Adapted tolerance of nonlinear solver so that it is identical to integrator tolerance (to avoid failed initialization for integrator tolerances smaller as 1e-6).Bugs in event iteration corrected:\nNumber of event iterations is limited (more as 20 iterations triggers an error), to avoid infinite looping.\nAlso at a time-event, re-initialization is performed to compute consistent x, der(x) variables.Documentation docs/src/man/Overview.md considerably improved. Now reflecting the current status of ModiaMath.In some examples corrected packages to import packages via ModiaMath.Using newest versions of all used packages."
},
...

{
    ...
    "category": "section",
    "text": "You can just past the following code fragments into the Julia REPL."
},
...

{
    ...
    "category": "section",
    "text": "  result = ModiaMath.simulate!(simulationModel; log=true);\r\n  ModiaMath.plot(result, [(:phi, :w) :a])This results in:<img src=\"../../resources/images/pendulumPlot.svg\">"
},
...
{
    "location": "man/Overview.html#To-run-a-multi-mode-example-that-has-Dirac-impulses-1",
    "page": "Overview",
    "title": "To run a multi-mode example that has Dirac impulses",
    "category": "section",
    "text": "The following model from [3]<img src=\"../../resources/images/idealClutchDiagram.png\">is defined by an ideal clutch described by two modes: Released (the clutch shafts rotate freely) and Engaged (the clutch shafts are rigidly fixed). If variable engaged = true, the clutch is in mode Engaged and otherwise in mode Released. An event occurs when Boolean variable engaged changes its value. If the clutch is in mode Released and changes to Engaged a Dirac impulse occurs (the clutch torque becomes infinite) and changes the clutch shaft speeds  (inertia1.w, inertia2.w) in such a way that they coincide. Since inertia1.w is algebraically  coupled to capacitor.v (the voltage over the capacitor) via the emf element, also   capacitor.v changes discontinuously and der(capacitor.v) is a Dirac impulse.  Mathematically, this means that the DAE index is changing depending on the clutch state  and an  f_c equation is changed to an f_d equation and vice versa. This model can be simulated and the result plotted with command:import ModiaMath\r\ninclude(\"$(ModiaMath.path)/examples/withoutMacros_withoutVariables/Simulate_IdealClutch.jl\")\r\n  <img src=\"../../resources/images/idealClutchPlot.svg\">The results of the simulation are shown in the figure above. The initial speed of inertia1 is w1 = 0 rad/s and w2 = 10 rad/s for inertia2. The capacitor is initially uncharged. The upper two partly overlapping curves are the angular velocities of inertia1 and inertia2. When the clutch is disengaged, the angular velocity of inertia2 is constant. The green curve shows the voltage over the capacitor. Since the clutch is engaged at initialization, Dirac impulses occur at time = 0 s. As a result, after initialization the angular velocities of the two inertias are identical. The common angular velocity at time = 0 s is 6.4 rad/s. The reason is that the capacitor acts in the same way as an additional moment of inertia to inertia1. The effective inertia is J_eff = J1 + J2 + k^2 C = 01 + 04 + 025^2 cdot 2 = 0625Thus, the common angular velocity for the two inertias at time=0 s becomes beginalign\r\nw^+_0 = (J1 cdot w1^-_0 + J2 cdot w2^-_0 + k cdot C cdot v^-_0)J_eff \r\n        = (01cdot0 + 04cdot10 + 025cdot2cdot0)0625 = 64 text rads\r\nendalignThere are no Dirac impulses at time = 100 s when the clutch disengages, but again Dirac impulses at time = 300 s when the clutch engages again."
},
...
{
    "location": "lib/DAE.html#ModiaMath.DAE.compute_der_fc-Tuple{ModiaMath.DAE.SimulationState}",
    "page": "DAE",
    "title": "ModiaMath.DAE.compute_der_fc",
    "category": "method",
    "text": "ModiaMath.compute_der_fc(m::ModiaMath.[AbstractSimulationModel|SimulationState])\n\nIf true is returned, return the derivative of the constraint equation in residues r[i].  Otherwise return constraint equations.\n\n\n\n\n\n"
},
...
{
    "location": "lib/DAE.html#ModiaMath.DAE.get_is_constraint-Tuple{ModiaMath.DAE.SimulationState}",
    "page": "DAE",
    "title": "ModiaMath.DAE.get_is_constraint",
    "category": "method",
    "text": "ModiaMath.get_is_constraint(m::ModiaMath.[AbstractSimulationModel|SimulationState])\n\nReturn reference to is_constraint vector (this vector can be modified in getModelResidues! if ModiaMath.isEvent returns true.\n\n\n\n\n\n"
},
